...
def clean_data(gt_anno, dt_anno, current_class, difficulty, custom_method=0):
    ...
    gt_anno['difficulty'] = np.zeros(num_gt)
    for i in range(num_gt):
        # add to just one difficulty
        if custom_method == 1:
            if gt_anno['num_radar_points_in_gt'][i] == 0:
                gt_anno["difficulty"][i] = -1
        if gt_anno["difficulty"][i] >= 0:
            range_gt = (gt_anno["location"][i, 0] ** 2 + gt_anno["location"][i, 1] ** 2 + gt_anno["location"][i, 2] ** 2) ** 0.5
            if custom_method != 2:
        ...
        if (gt_name == current_cls_name):
            ...
            valid_class = -1
        ignore = False
        if (custom_method != 2 and (gt_anno["difficulty"][i] > difficulty or gt_anno["difficulty"][i] == -1)) or \
                (custom_method == 2 and (gt_anno["difficulty"][i] != difficulty or gt_anno["difficulty"][i] == -1)):
            ignore = True
...
def do_eval(gt_annos, dt_annos, current_classes, min_overlaps, eval_types=['bbox', 'bev', '3d'], custom_method=0):
    # min_overlaps: [num_minoverlap, metric, num_class]
    difficultys = [0, 1, 2]
    mAP_bbox = None
    mAP_aos = None
    if 'bbox' in eval_types:
        ret = eval_class(gt_annos, dt_annos, current_classes, difficultys, 0, min_overlaps, compute_aos=('aos' in eval_types), custom_method=custom_method)
    ...
    if 'bev' in eval_types:
        ret = eval_class(gt_annos, dt_annos, current_classes, difficultys, 1, min_overlaps, custom_method=custom_method)
        mAP_bev = get_mAP(ret['precision'])

    mAP_3d = None
    if '3d' in eval_types:
        ret = eval_class(gt_annos, dt_annos, current_classes, difficultys, 2, min_overlaps, custom_method=custom_method)
        mAP_3d = get_mAP(ret['precision'])
    return mAP_bbox, mAP_bev, mAP_3d, mAP_aos


def TJ4DRadSet_eval(gt_annos, dt_annos, current_classes, eval_types=['bbox', 'bev', '3d'], custom_method=0):
    """KITTI evaluation.
    ...
    lap1 = 0.5
    lap2 = 0.25
    overlap_0_7 = np.array([[lap1] * 4, [lap1] * 4, [lap1] * 4])
    overlap_0_5 = np.array([[lap2] * 4, [lap2] * 4, [lap2] * 4])
    ...
    mAPbbox, mAPbev, mAP3d, mAPaos = do_eval(gt_annos, dt_annos, current_classes, min_overlaps, eval_types, custom_method=custom_method)
    ...
    if len(current_classes) > 1:
        ...
        result += ('\nOverall AP40@{}, {}, {}:\n'.format(*difficulty))
        if mAPbbox is not None:
            mAPbbox = np.concatenate((mAPbbox[:1, :, 0], mAPbbox[1:3, :, 1], mAPbbox[3:, :, 0]))
            mAPbbox = mAPbbox.mean(axis=0)
            result += 'bbox AP:{:.4f}, {:.4f}, {:.4f}\n'.format(*mAPbbox)
        if mAPbev is not None:
            mAPbev = np.concatenate((mAPbev[:1, :, 0], mAPbev[1:3, :, 1], mAPbev[3:, :, 0]))
            mAPbev = mAPbev.mean(axis=0)
            result += 'bev  AP:{:.4f}, {:.4f}, {:.4f}\n'.format(*mAPbev)
        if mAP3d is not None:
            mAP3d = np.concatenate((mAP3d[:1, :, 0], mAP3d[1:3, :, 1], mAP3d[3:, :, 0]))
            mAP3d = mAP3d.mean(axis=0)
            result += '3d   AP:{:.4f}, {:.4f}, {:.4f}\n'.format(*mAP3d)
        if compute_aos:
            mAPaos = np.concatenate((mAPaos[:1, :, 0], mAPaos[1:3, :, 1], mAPaos[3:, :, 0]))
        ...
        for idx in range(3):
            ...
            if mAP3d is not None:
                ret_dict[f'Overall_mAP_3D_{postfix}'] = mAP3d[idx]
            if mAPbev is not None:
                ret_dict[f'Overall_mAP_BEV_{postfix}'] = mAPbev[idx]
            if mAPbbox is not None:
                ret_dict[f'Overall_mAP_2D_{postfix}'] = mAPbbox[idx]
...
def d3_box_overlap_kernel(boxes, qboxes, rinc, criterion=-1):
    ...
    for i in numba.prange(N):
        for j in numba.prange(K):
            if rinc[i, j] > 0:
                # iw = (min(boxes[i, 1] + boxes[i, 4], qboxes[j, 1] + qboxes[j, 4]) - max(boxes[i, 1], qboxes[j, 1]))
...
def fused_compute_statistics(overlaps, pr, gt_nums, dt_nums, dc_nums, gt_datas, dt_datas, dontcares, ignored_gts,
    ...
    for i in range(gt_nums.shape[0]):
        for t, thresh in enumerate(thresholds):
            ...
            tp, fp, fn, similarity, _ = compute_statistics_jit(overlap, gt_data, dt_data, ignored_gt, ignored_det,
                dontcare, metric, min_overlap=min_overlap, thresh=thresh, compute_fp=True, compute_aos=compute_aos)
            pr[t, 0] += tp
            pr[t, 1] += fp
            pr[t, 2] += fn
            if similarity != -1:
...
def _prepare_data(gt_annos, dt_annos, current_class, difficulty, custom_method=0):
    ...
    for i in range(len(gt_annos)):
        rets = clean_data(gt_annos[i], dt_annos[i], current_class, difficulty, custom_method=custom_method)
...
def eval_class(gt_annos, dt_annos, current_classes, difficultys, metric, min_overlaps, compute_aos=False, num_parts=200, custom_method=0):
            ...
            for k, min_overlap in enumerate(min_overlaps[:, metric, m]):
                ...
                for i in range(len(gt_annos)):
                    rets = compute_statistics_jit(overlaps[i], gt_datas_list[i], dt_datas_list[i], ignored_gts[i],
                        ignored_dets[i], dontcares[i], metric, min_overlap=min_overlap, thresh=0.0, compute_fp=False)
                ...
                for j, num_part in enumerate(split_parts):
                    ...
                    fused_compute_statistics(parted_overlaps[j], pr,
                        ...
                        gt_datas_part, dt_datas_part, dc_datas_part, ignored_gts_part, ignored_dets_part, metric,
                        min_overlap=min_overlap, thresholds=thresholds, compute_aos=compute_aos)
                    idx += num_part
